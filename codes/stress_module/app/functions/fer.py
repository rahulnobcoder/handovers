        ...
        for frame in tqdm(frames):
            ...
            if frame is not None:
                ...
                with torch.no_grad():
                    ...
                    self.em.append(emotion)
                self.emotion_count[predicted.item()] += 1
                label = f"{self.label_dict[predicted.item()]}"
                self.emotion_list.append(label)
            else:
                self.em.append('fnf')
                self.emotion_list.append('fnf')
            self.frame_count+=1
        
    def get_df(self):
        data = {
        "timestamp": self.time_stamps,
        "emotion": self.emotion_list,
        }
        self.df = pd.DataFrame(data)
        self.class_wise_frame_count = dict(zip(self.class_labels, self.emotion_count))
        return self.df,self.class_wise_frame_count,self.em

def fer_predict(video_frames,fps,fer_model):
    model=Model(fps,fer_model)
    model.predict(video_frames)
    return model.get_df()

import matplotlib.pyplot as plt
def plot_graph(values,var,path):
    x,y=values
