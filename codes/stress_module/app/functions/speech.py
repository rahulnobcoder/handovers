...
def predict_emotions(audio_path, interval,model_s):
    ...
    for start in np.arange(0, duration, interval):
        ...
        if feat is not None:
            ...
            emotions.append([start, label_mapping[predictions[0]]])
    return emotions
...
def generate_word_cloud(word_weights):
    if len(word_weights)>0:
        return word_weights
def analyze_audio(file_path,valence_dict_path,arousal_dict_path,dominance_dict_path):
...
def speech_predict(audio_path,model_path,valence_dict_path,arousal_dict_path,dominance_dict_path,word_path):
    ...
    emotions_df = pd.DataFrame(emotions, columns=["timestamp", "emotion"],dtype='object')
    
    # Save emotions to a log file
    # Extrapolate major emotions
    major_emotion = emotions_df['emotion'].mode().values[0]    
    word = analyze_audio(audio_path,valence_dict_path,arousal_dict_path,dominance_dict_path)
    df=pd.DataFrame(word['mapped_values'],columns=['word','valence','arousal','dominance','flag'])
